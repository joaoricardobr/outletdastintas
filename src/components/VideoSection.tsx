...
export default function VideoSection() {
  ...
  return (
    <section id="videos" style={{ width: "100%", padding: "96px 0", background: "#070707", position: "relative", overflow: "hidden", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
        ...
        <div style={{ textAlign: "center", maxWidth: 560, margin: "0 auto 56px" }}>
          ...
          <p style={{ color: "#777", marginTop: 14, fontWeight: 300, fontSize: 15, lineHeight: 1.7 }}>
            Acompanhe processos reais de aplicação, consultoria e resultado final dos nossos projetos de alto padrão.
          </p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 24 }}>
          ...
          <div style={{ gridColumn: "span 2" }} className="feat-col">
            <VideoCard v={featured} onPlay={() => setActive(featured.youtubeId)} hov={hov === featured.id} onHov={() => setHov(featured.id)} onLeave={() => setHov(null)} featured />
          </div>
          {/* secondary */}
          {secondary.map(v => (
            <VideoCard key={v.id} v={v} onPlay={() => setActive(v.youtubeId)} hov={hov === v.id} onHov={() => setHov(v.id)} onLeave={() => setHov(null)} />
          ))}
        </div>
      </div>
      ...
      {/* Modal */}
      {active && (
        <div onClick={() => setActive(null)} style={{ position: "fixed", inset: 0, zIndex: 999, background: "rgba(0,0,0,0.94)", backdropFilter: "blur(12px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div onClick={e => e.stopPropagation()} style={{ position: "relative", width: "100%", maxWidth: 900, aspectRatio: "16/9", borderRadius: 16, overflow: "hidden", background: "#000", boxShadow: "0 0 80px rgba(0,0,0,0.8)" }}>
            <button onClick={() => setActive(null)} style={{ position: "absolute", top: 12, right: 12, zIndex: 10, background: "rgba(0,0,0,0.7)", borderWidth: 1, borderStyle: "solid", borderColor: "rgba(255,255,255,0.15)", borderRadius: "50%", width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#fff" }}>
              <X size={18} />
            </button>
            <iframe
...
function VideoCard({ v, onPlay, hov, onHov, onLeave, featured }: {
  v: typeof videos[0]; onPlay: () => void; hov: boolean; onHov: () => void; onLeave: () => void; featured?: boolean;
}) {
  return (
    <div onMouseEnter={onHov} onMouseLeave={onLeave}
      style={{ borderRadius: 16, overflow: "hidden", position: "relative", cursor: "pointer", borderWidth: 1, borderStyle: "solid", borderColor: hov ? "rgba(212,175,55,0.3)" : "rgba(255,255,255,0.08)", transition: "border-color 0.3s, box-shadow 0.3s", boxShadow: hov ? "0 20px 60px rgba(0,0,0,0.5)" : "none" }}>
      {/* thumbnail */}
      <div style={{ position: "relative", aspectRatio: featured ? "16/8" : "16/9", overflow: "hidden" }}>
        ...
        {/* duration badge */}
        <div style={{ position: "absolute", top: 14, right: 14, padding: "4px 10px", background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)", borderRadius: 6, fontSize: 11, color: "#ccc", letterSpacing: "0.1em" }}>{v.duration}</div>
        {/* play button */}
        <button onClick={onPlay}
          style={{ position: "absolute", top: "50%", left: "50%", transform: `translate(-50%,-50%) scale(${hov ? 1.1 : 1})`, transition: "transform 0.3s", width: featured ? 72 : 56, height: featured ? 72 : 56, borderRadius: "50%", background: hov ? "rgba(212,175,55,0.9)" : "rgba(255,255,255,0.12)", backdropFilter: "blur(8px)", borderWidth: 2, borderStyle: "solid", borderColor: hov ? "#D4AF37" : "rgba(255,255,255,0.25)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: hov ? "0 0 32px rgba(212,175,55,0.5)" : "none" }}>
      ...
      <div style={{ padding: "16px 20px", background: "#0d0d0d" }}>
        <p style={{ fontSize: 12, color: "#777", lineHeight: 1.65 }}>{v.desc}</p>
      </div>
    </div>
  );
}
